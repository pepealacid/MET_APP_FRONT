...
const metMap =
  "https://maps.metmuseum.org/?screenmode=base&floor=1#hash=17.98/40.77923/-73.962729/-61";
    ...
    if (data.path) {
      const objectData = data.path.map((gallery) => {
        ...
        data.artworkData.forEach((artPiece) => {
          artPiece.GalleryNumber == gallery ? artPieces.push(artPiece) : null;
        });
        return {
          gallery,
          artPieces,
  ...
  return (
    t?.tours && (
      <Box>
        ...
        {finalTour && <TourCard currentGallery={finalTour[swipeCount]} />}

        <AspectRatio h="400px" ratio={4 / 9}>
          <iframe src={metMap} />
        </AspectRatio>
      </Box>
    )
