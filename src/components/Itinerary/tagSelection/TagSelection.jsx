...
function TagSelection({
  ...
  const handleSend = (event) => {
    if (selectedDepartments.length) {
      ...
      setMessage(t?.tours.oneTag || "You have to choose at least one tag");
    }
  };
      ...
      <>
        <Box w="100%" h="100vh" bg={`#f5f2f2`}>
          <Box minH="calc(100vh - 100px)">
            ...
            <Wrap w="400px" justify="center" pl="20px" mt="40px">
              {departments.map(({ departmentId, displayName }) => (
                <WrapItem
                  ...
                  key={departmentId}
                  onClick={(e) => {
                    handleSelection(e, departmentId, displayName);
                  }}
                >
            ...
            <Flex p={4}>
              <Checkbox checked={onlyHighlights} />{" "}
              <Text size="sm">
                {t?.tours.important ||
                  "I only want to see the most important artworks"}
              </Text>
            </Flex>
